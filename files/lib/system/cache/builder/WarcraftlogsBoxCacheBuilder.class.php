...
class WarcraftlogsBoxCacheBuilder extends AbstractCacheBuilder {
	...
	protected function rebuild(array $parameters){
		...
		try{			
			...
			$strGuildname = WARCRAFTLOGS_GUILDNAME;
			$strServername = WARCRAFTLOGS_SERVERNAME;
			$strServerregion = WARCRAFTLOGS_REGION;
			$strAPIKey = WARCRAFTLOGS_APIKEY;
			
			$strServername = mb_strtolower($strServername);
			$strServername = str_replace(' ', '-', $strServername);
			$strServername = str_replace("'", '', $strServername);
			$strServername = $this->remove_accents($strServername);
			
			$strReportsURL = $url."/v1/reports/guild/".rawurlencode($strGuildname)."/".$strServername."/".$strServerregion."?api_key=".$strAPIKey;
			
			//start request
			$request = new HTTPRequest($strReportsURL);
			$request->execute();
